class UI {
    ...
    // Gera script de automacao para o site da Caixa - v4.2 (robusto + modal handler)
    _generateCaixaScript_LEGACY(config, games) {
        const gamesJSON = JSON.stringify(games);
        const lName = config.name;
        return '(async function(){var JOGOS=' + gamesJSON + ';var TOTAL=JOGOS.length;var OK=0;var ERROS=0;console.clear();console.log("B2B v4.2 "+TOTAL+" jogos de ' + lName + '");var delay=function(ms){return new Promise(function(r){setTimeout(r,ms)})};function fecharModais(){var modais=document.querySelectorAll(".modal-content .close,.modal .btn-close,button.close,[class*=close-modal],[class*=modal] button,[class*=dialog] button,.alert button,.swal2-confirm,.swal2-close");for(var i=0;i<modais.length;i++){try{if(modais[i].offsetParent!==null){modais[i].click();console.log("Modal fechado")}}catch(e){}}var okBtns=document.querySelectorAll("button");for(var j=0;j<okBtns.length;j++){var t=okBtns[j].textContent.trim().toLowerCase();if((t==="ok"||t==="confirmar"||t==="sim"||t==="fechar")&&okBtns[j].offsetParent!==null){try{okBtns[j].click();console.log("Btn ok/confirmar fechado")}catch(e){}}}}function clicarNumero(num){var p=String(num).padStart(2,"0");var el=document.getElementById("n"+p);if(el){el.click();return true}el=document.querySelector("a#n"+p);if(el){el.click();return true}var todos=document.querySelectorAll(".numbers li,.numbers a,.numero,.dezena,[class*=number] li,[class*=number] a,li a,.collection-lottery li a");for(var i=0;i<todos.length;i++){var t=todos[i].textContent.trim();if(t===p||t===String(num)){todos[i].click();return true}}var all=document.querySelectorAll("a,span,li");for(var j=0;j<all.length;j++){if(all[j].children.length===0&&all[j].textContent.trim()===p){all[j].click();return true}}console.warn("Num "+p+" nao achado");return false}function limpar(){fecharModais();var btn=document.getElementById("limparvolante");if(btn){btn.click();return}btn=document.querySelector("[id*=limpar]");if(btn){btn.click();return}var bs=document.querySelectorAll("button,a,span");for(var i=0;i<bs.length;i++){if(bs[i].textContent.toLowerCase().includes("limpar")){bs[i].click();return}}}async function carrinho(){fecharModais();for(var t=0;t<3;t++){var btn=document.getElementById("colocarnocarrinho");if(!btn)btn=document.querySelector("[id*=colocarnocarrinho]");if(!btn)btn=document.querySelector("[id*=carrinho]");if(btn&&btn.textContent&&!btn.textContent.toLowerCase().includes("ir para")){btn.click();await delay(1000);fecharModais();await delay(500);return true}var bs=document.querySelectorAll("button,a");for(var i=0;i<bs.length;i++){var tx=bs[i].textContent.toLowerCase();if(tx.includes("colocar no carrinho")||(tx.includes("carrinho")&&!tx.includes("ir para"))){bs[i].click();await delay(1000);fecharModais();await delay(500);return true}}console.warn("Tent "+(t+1)+"/3");await delay(2000);fecharModais()}return false}for(var i=0;i<TOTAL;i++){var jogo=JOGOS[i];console.log("Jogo "+(i+1)+"/"+TOTAL+" ["+jogo.join(", ")+"]");fecharModais();if(i>0){limpar();await delay(1500);fecharModais();await delay(500)}var ac=0;for(var n=0;n<jogo.length;n++){if(clicarNumero(jogo[n]))ac++;await delay(300)}if(ac<jogo.length)console.warn("Jogo "+(i+1)+": "+ac+"/"+jogo.length);await delay(1500);fecharModais();var ok=await carrinho();if(ok){OK++;console.log("OK Jogo "+(i+1)+" ("+OK+"/"+TOTAL+")")}else{ERROS++;console.error("FALHOU Jogo "+(i+1))}if(i<TOTAL-1)await delay(4000);if((i+1)%10===0)console.log("Progresso "+(i+1)+"/"+TOTAL+" ("+Math.round((i+1)/TOTAL*100)+"%)")}console.log("PRONTO! "+OK+"/"+TOTAL+" jogos de ' + lName + ' no carrinho!");alert("Pronto! "+OK+"/"+TOTAL+" jogos de ' + lName + '!"+(ERROS>0?"\\n"+ERROS+" com erro.":"")+"\\nFinalize o pagamento.")})()';
    }
