class UI {
    ...
    // Gera script de automacao para o site da Caixa - v7.0 TURBO (Blocos de 10 + Velocidade 3x)
    _generateCaixaScript_LEGACY(config, games) {
        const gamesJSON = JSON.stringify(games);
        const lName = config.name;
        return '(async function(){var JOGOS=' + gamesJSON + ';var TOTAL=JOGOS.length;var OK=0;var ERROS=0;var BLOCO=10;console.clear();console.log("%c[B2B v7.0 TURBO] "+TOTAL+" jogos de ' + lName + '","color:#22C55E;font-size:14px;font-weight:bold");console.log("%cProcessando em blocos de "+BLOCO+"...","color:#60A5FA");var t0=Date.now();var delay=function(ms){return new Promise(function(r){setTimeout(r,ms)})};function fecharModais(){var ids=["fecharModalAlerta","fecharModalErro","fecharModalInfo","confirmarModalConfirmacao","botaosim","btnFecharModal","btnOk","btnConfirmar"];for(var k=0;k<ids.length;k++){var el=document.getElementById(ids[k]);if(el&&el.offsetParent!==null&&el.offsetWidth>0){try{el.click()}catch(e){}}}var bts=document.querySelectorAll("button,a");for(var j=0;j<bts.length;j++){var t=bts[j].textContent.trim().toLowerCase();if((t==="ok"||t==="entendi"||t==="fechar"||t==="confirmar"||t==="sim"||t==="continuar")&&bts[j].offsetParent!==null&&bts[j].offsetWidth>0){var isCart=bts[j].id&&bts[j].id.toLowerCase().includes("carrinho");if(!isCart){try{bts[j].click()}catch(e){}}}}}function clicarNumero(num){var p=String(num).padStart(2,"0");var el=document.getElementById("n"+p);if(el){el.click();return true}el=document.querySelector("a#n"+p);if(el){el.click();return true}var todos=document.querySelectorAll("a[role=button],a.dezena,a.numero");for(var i=0;i<todos.length;i++){if(todos[i].textContent.trim()===p){todos[i].click();return true}}var allN=document.querySelectorAll(".number,.dezena,.num,[class*=number],[class*=dezena]");for(var k=0;k<allN.length;k++){var txt=allN[k].textContent.trim();if(txt===p||txt===String(num)){allN[k].click();return true}}return false}function limpar(){fecharModais();var btn=document.getElementById("limparvolante");if(btn){btn.click();return true}btn=document.querySelector("[id*=limpar]");if(btn){btn.click();return true}var bts=document.querySelectorAll("button");for(var k=0;k<bts.length;k++){if(bts[k].textContent.toLowerCase().includes("limpar")){bts[k].click();return true}}return false}async function carrinho(){fecharModais();await delay(200);for(var t=0;t<5;t++){var btn=document.getElementById("colocarnocarrinho");if(btn&&btn.offsetParent!==null){btn.click();await delay(1200);fecharModais();await delay(500);return true}var allB=document.querySelectorAll("button");for(var k=0;k<allB.length;k++){var tx=allB[k].textContent.toLowerCase().trim();if(tx.includes("colocar no carrinho")||(tx.includes("carrinho")&&!tx.includes("ir para")&&!tx.includes("ver"))){allB[k].click();await delay(800);fecharModais();await delay(400);return true}}fecharModais();await delay(600)}return false}fecharModais();await delay(500);for(var i=0;i<TOTAL;i++){var jogo=JOGOS[i];var blocoN=Math.floor(i/BLOCO)+1;var blocoT=Math.ceil(TOTAL/BLOCO);if(i%BLOCO===0)console.log("%c=== BLOCO "+blocoN+"/"+blocoT+" ===","color:#F59E0B;font-weight:bold;font-size:12px");console.log("[B2B] "+(i+1)+"/"+TOTAL+" ["+jogo.join(",")+"]");fecharModais();if(i>0){limpar();await delay(1000);fecharModais();await delay(300)}var ac=0;for(var n=0;n<jogo.length;n++){if(clicarNumero(jogo[n]))ac++}if(ac<jogo.length)console.warn("[B2B] Jogo "+(i+1)+": "+ac+"/"+jogo.length);await delay(1000);fecharModais();await delay(300);var ok=await carrinho();if(ok){OK++;console.log("[B2B] OK "+(i+1)+" ("+OK+"/"+TOTAL+")")}else{ERROS++;console.error("[B2B] FALHOU "+(i+1))}if(i<TOTAL-1){await delay(1200);fecharModais()}if((i+1)%BLOCO===0){var el=((Date.now()-t0)/1000).toFixed(0);var sp=((i+1)/el*60).toFixed(0);console.log("%c[B2B] Bloco "+blocoN+" OK! "+OK+"/"+TOTAL+" | "+el+"s | ~"+sp+" jogos/min","color:#22C55E;font-weight:bold");if(i+1<TOTAL){console.log("[B2B] Proximo bloco em 2s...");await delay(2000)}}}var tt=((Date.now()-t0)/1000).toFixed(1);console.log("%c[B2B v7.0] PRONTO! "+OK+"/"+TOTAL+" de ' + lName + ' em "+tt+"s","color:#22C55E;font-size:16px;font-weight:bold");alert("[B2B v7.0 TURBO] "+OK+"/"+TOTAL+" jogos de ' + lName + ' em "+tt+"s!"+(ERROS>0?"\\n"+ERROS+" erro(s).":"")+"\\nFinalize o pagamento.")})()';
    }
