    ...
    runQuantumCalculation() {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                statusDiv.innerHTML = '<div class="quantum-placeholder" style="opacity: 1; color: #ec4899; font-style: normal;">â˜… AnÃ¡lise 21 Camadas + Walk-Forward â†’ PrÃ³ximo Sorteio...</div>';
                ...
                setTimeout(() => {
                    try {
                        ...
                        // â˜… v10.0: Calcular confianÃ§a via Walk-Forward real
                        if (suggestion && suggestion.length > 0 && history.length >= 5) {
                            try {
                                // Walk-Forward: testar sugestÃ£o contra Ãºltimos 10 sorteios
                                ...
                                console.log('[v10.0] Walk-Forward SugestÃ£o: avg=' + avgHits.toFixed(2) + ' vs acaso=' + expectedRandom.toFixed(2) + ' | ' + improvement.toFixed(2) + 'x | Conf=' + wfConfidence + '%');
                            } catch(btErr) {
            ...
            <div style="text-align:center;padding:30px;background:linear-gradient(145deg,rgba(10,10,30,0.95),rgba(20,10,40,0.9));border-radius:16px;border:1px solid rgba(139,92,246,0.3);">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin:12px auto;max-width:450px;">
                    <div id="q-phase-1" style="padding:10px 8px;border-radius:10px;background:rgba(139,92,246,0.15);border:1px solid rgba(139,92,246,0.3);transition:all 0.5s;">
                        ...
                        <div style="color:#C4B5FD;font-size:0.7rem;font-weight:700;">ANÃLISE</div>
                    ...
                    <div id="q-phase-2" style="padding:10px 8px;border-radius:10px;background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.15);transition:all 0.5s;opacity:0.4;">
                        ...
                        <div style="color:#10B981;font-size:0.6rem;">CALIBRAÃ‡ÃƒO</div>
                    </div>
                    <div id="q-phase-3" style="padding:10px 8px;border-radius:10px;background:rgba(236,72,153,0.08);border:1px solid rgba(236,72,153,0.15);transition:all 0.5s;opacity:0.4;">
                        ...
                        <div style="color:#EC4899;font-size:0.6rem;">VALIDAÃ‡ÃƒO</div>
                    </div>
                </div>
                <div id="q-status" style="color:#C4B5FD;font-weight:600;font-size:0.82rem;margin-top:8px;">âš›ï¸ Analisando 21 camadas estatÃ­sticas...</div>
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    try {
                            ...
                            let analysisHTML = `
                                <div class="smart-analysis-panel" style="margin-top:10px;margin-bottom:10px;padding:12px 16px;border-radius:12px;background:linear-gradient(145deg,rgba(15,23,42,0.95),rgba(30,41,59,0.9));border:1px solid ${confColor}40;box-shadow:0 4px 20px rgba(0,0,0,0.3);">
                                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;flex-wrap:wrap;gap:6px;">
                                        <span style="color:${confColor};font-weight:800;font-size:0.9rem;">${confEmoji} Walk-Forward: ${analysis.confidence}% (${confLabel}) ${analysis.monteCarlo != null ? (analysis.monteCarlo > 0 ? '<span style="color:#22C55E;font-size:0.75rem;">ðŸ“Š MC +' + analysis.monteCarlo + '% vs acaso</span>' : '<span style="color:#EAB308;font-size:0.75rem;">ðŸ“Š MC ' + analysis.monteCarlo + '% vs acaso</span>') : ''}</span>
                        ...
                        feedback.innerHTML = `<span style="color:#FFD700;font-size:1rem;">âš¡ L99 v10.0</span><br><span style="color:#C4B5FD;font-size:0.9rem;">${result.games.length} jogos â€” 21 Camadas + Walk-Forward + Monte Carlo</span>`;
...
// â”€â”€ DISCLAIMER â”€â”€
        ...
        try {
            if (typeof StatisticsTracker !== 'undefined') {
                ...
                var self = this;
                // â”€â”€ SALVAR CONFERÃŠNCIA NA PASTA (async para nÃ£o travar) â”€â”€
                setTimeout(function() { try {
                    var reportText = 'â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•\n';
                    reportText += '  ðŸ“Š CONFERÃŠNCIA L99 â€” ' + game.name + '\n';
                    reportText += '  Concurso ' + concursoNum + '\n';
                    reportText += '  Data: ' + new Date().toLocaleString('pt-BR') + '\n';
                    reportText += 'â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•\n\n';
                    reportText += 'ðŸ“Š RESUMO\n';
                    ...
                    reportText += '  NÂº por Jogo: ' + actualDrawSize + '\n\n';
                    reportText += 'ðŸ”¢ NÃšMEROS SORTEADOS: ' + drawnNumbers.join(' - ') + '\n\n';
                    reportText += 'ðŸ… FAIXAS DE PREMIAÃ‡ÃƒO\n';
                    ...
                    if (winningGames.length > 0) {
                        reportText += 'ðŸ† JOGOS GANHADORES\n';
                        winningGames.forEach(function(w) {
                            reportText += '  Jogo ' + (w.index + 1) + ': ' + w.numbers.join(' - ') + ' â†’ ' + w.hits + ' acertos (' + w.strat.label + ')\n';
                        });
                    }
                    var maxListJogos = Math.min(self.currentGeneratedGames.length, 200);
                    reportText += '\nðŸ“‹ JOGOS CONFERIDOS (' + maxListJogos + ' de ' + self.currentGeneratedGames.length + ')\n';
                    ...
                    reportText += '  L99 B2B Loterias â€” RelatÃ³rio AutomÃ¡tico\n';
                    reportText += 'â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•\n';
                    fetch('/salvar-conferencia', {
                        ...
                        if (result.ok) console.log('[L99] âœ… ConferÃªncia salva em: ' + result.fileName);
                    }).catch(function() {});
                    // Marcar arquivo original como conferido
                    if (self._lastLoadedFileName && !self._lastLoadedFileName.startsWith('âœ…')) {
                    ...
                    console.warn('[L99] Erro ao salvar conferÃªncia:', saveErr);
                } }, 100); // setTimeout
            }
        } catch(statsErr) {
            console.warn('[Stats] Erro ao salvar estatÃ­stica:', statsErr);
        }
    }
    exportBackup() {
        ...
        if (this.fixedNumbers.size > 0) {
            ...
            content += `  NÃºmeros Fixos: ${fixedArr.map(n => n.toString().padStart(2, '0')).join(' ')}\n`;
        }
        ...
        if (h3) {
            ...
            h3.textContent = 'âœ… BACKUP SALVO!';
    ...
    // â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
    // â•‘  PAINEL ESTATÃSTICAS â€” ConferÃªncia de Resultados & ROI     â•‘
    // â•‘  Rastreia performance de apostas para calibrar o motor      â•‘
    // â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        ...
        if (exportBtn) {
            exportBtn.addEventListener('click', () => {
                try {
                    ...
                    document.body.removeChild(a);
                    URL.revokeObjectURL(url);
                    exportBtn.textContent = 'âœ… Exportado!';
                    setTimeout(() => exportBtn.textContent = 'ðŸ“¥ Exportar CSV', 2000);
                } catch(e) {
            ...
            importInput.addEventListener('change', (e) => {
                ...
                reader.onload = (ev) => {
                    try {
                        ...
                        alert(`âœ… ${count} registros importados com sucesso!`);
                        this._buildStatsLotteryGrid();
        ...
        if (clearBtn) {
            clearBtn.addEventListener('click', () => {
                const lName = this._statsActiveLottery
                    ...
                    : 'TODAS AS LOTERIAS';
                if (confirm(`âš ï¸ Tem certeza que deseja limpar TODOS os dados de estatÃ­sticas de ${lName}?\n\nEsta aÃ§Ã£o nÃ£o pode ser desfeita.`)) {
    ...
    // â”€â”€ Construir Grid de Loterias â”€â”€
    ...
    // â”€â”€ Renderizar Tabela de EstatÃ­sticas â”€â”€
    renderStatisticsTable(lotteryKey) {
        ...
        this._populateStatsFilter('stats-filter-numjogo', 'NÂº/Jogo', allRecords.map(r => r.numPorJogo), filters.numPorJogo);

        // â”€â”€ Sem dados â”€â”€
        if (records.length === 0) {
            tableArea.innerHTML = `
                <div class="stats-empty">
                    <div class="stats-empty-icon">ðŸ“­</div>
                    ...
                    <div class="stats-empty-desc">
                        ${allRecords.length > 0
                            ...
                            : `Ainda nÃ£o hÃ¡ conferÃªncias registradas para <strong>${game.name}</strong>.<br>Gere jogos, depois use o botÃ£o <strong>"Conferir"</strong> para verificar os resultados.`}
                    </div>
                </div>`;
        ...
        if (totalsBar) {
            ...
            totalsBar.innerHTML = `
                <div class="stats-total-card"><div class="stats-total-label">ConferÃªncias</div><div class="stats-total-value">${agg.total}</div></div>
                ...
                <div class="stats-total-card"><div class="stats-total-label">Retorno MÃ©dio</div><div class="stats-total-value ${retClass}">${agg.pctRetornoMedio >= 0 ? '+' : ''}${agg.pctRetornoMedio.toFixed(1)}%</div></div>
            `;
        }
        ...
        // â”€â”€ Adicionar Banner de PremiaÃ§Ã£o (Timemania / Dia de Sorte) â”€â”€
        let prizeBannerHtml = '';
            ...
            if (prizeInfo && prizeInfo.estimatedPrize) {
                ...
                prizeBannerHtml = `
                    <div style="margin-bottom:15px;padding:12px 16px;background:linear-gradient(145deg,rgba(16,185,129,0.1),rgba(15,23,42,0.9));border:1px solid #10B98150;border-radius:8px;display:flex;align-items:center;gap:12px;">
                        ...
                        <div>
                            <div style="color:#10B981;font-weight:700;font-size:1.1rem;">PremiaÃ§Ã£o Estimada ${game.name}</div>
                            <div style="color:#E2E8F0;font-size:0.9rem;">PrÃ³ximo Concurso: <span style="font-weight:800;color:#FFD700;">${prizeFormatted}</span></div>
                        </div>
                    </div>
                `;
            }
        ...
        const extrasColLabel = isTimemania ? 'âš½ Time' : 'ðŸ“… MÃªs';
        ...
        // â˜… v9.5: Coluna Time/MÃªs para Timemania e Dia de Sorte
        if (hasExtrasCol) html += `<th>${extrasColLabel}</th>`;
        html += '<th>Valor PrÃªmio</th>';
        ...
        records.forEach(r => {
            const data = r.data ? new Date(r.data).toLocaleDateString('pt-BR') : 'â€”';
            ...
            const precBadge = r.precisao
                ...
                : '<span class="stats-badge nao">NÃ£o</span>';
            ...
            if (hasExtrasCol) {
                if (r.extrasGanhadores && r.extrasGanhadores.length > 0) {
                    ...
                    const icon = isTimemania ? 'âš½' : 'ðŸ“…';
                    const badges = r.extrasGanhadores.map(eg =>
                        `<span style="display:inline-block;font-size:0.58rem;color:${badgeColor};background:${badgeBg};padding:1px 4px;border-radius:3px;margin:1px;white-space:nowrap;" title="Jogo ${eg.jogo}: ${eg.acertos} acertos â‰ˆ R$${(eg.premio||0).toFixed(2)}">${icon} ${eg.extra}</span>`
                    ).join('');
            ...
            html += `<td><button class="stats-delete-btn" data-id="${r.id}" data-lottery="${lotteryKey}" title="Excluir registro">ðŸ—‘ï¸</button></td>`;
            html += '</tr>';
        });
    ...
    // â”€â”€ Popular filtro dinÃ¢mico â”€â”€
