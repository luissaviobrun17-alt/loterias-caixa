...
async function waitVolante(){for(var w=0;w<30;w++){var el=document.getElementById("n01")||document.querySelector("a[id^=n]");if(el&&el.offsetParent!==null){console.log("[B2B] Volante encontrado!");return!0}fm();await d(1000)}return!1}
async function navLoteria(){var hash=window.location.hash||"";if(hash.indexOf(LOTTERY_URL)>=0){console.log("[B2B] Ja esta na pagina correta");var ok=await waitVolante();if(ok)return!0}console.log("[B2B] Navegando para "+LOTTERY_URL+"...");var links=document.querySelectorAll("a");for(var i=0;i<links.length;i++){var href=links[i].getAttribute("href")||"";var txt=links[i].textContent.trim().toLowerCase();if(href.indexOf(LOTTERY_URL)>=0||(LOTTERY_URL==="mega-sena"&&txt.indexOf("mega")>=0&&txt.indexOf("sena")>=0)||(LOTTERY_URL==="lotofacil"&&txt.indexOf("lotof")>=0)||(LOTTERY_URL==="quina"&&txt.indexOf("quina")>=0)||(LOTTERY_URL==="dupla-sena"&&txt.indexOf("dupla")>=0)||(LOTTERY_URL==="lotomania"&&txt.indexOf("lotomania")>=0)||(LOTTERY_URL==="timemania"&&txt.indexOf("timemania")>=0)||(LOTTERY_URL==="dia-de-sorte"&&txt.indexOf("dia")>=0&&txt.indexOf("sorte")>=0)){console.log("[B2B] Clicando link: "+txt);rc(links[i]);await d(4000);fm();await d(1000);var ok=await waitVolante();if(ok)return!0}}console.log("[B2B] Tentando via hash...");window.location.hash="#/"+LOTTERY_URL;await d(4000);fm();await d(1000);return await waitVolante()}
