...
async function waitVolante(){for(var w=0;w<30;w++){var el=document.getElementById("n01")||document.querySelector("a[id^=n]");if(el&&el.offsetParent!==null)return!0;fm();await d(1000)}return!1}
async function navLoteria(){var hash=window.location.hash||"";if(hash.indexOf(LOTTERY_URL)<0){console.log("[B2B] Navegando para "+LOTTERY_URL+"...");window.location.hash="#!/"+LOTTERY_URL;await d(3000);fm();await d(1000);fm();}var ok=await waitVolante();if(!ok){console.log("[B2B] Tentando navegar novamente...");window.location.hash="#!/"+LOTTERY_URL;await d(3000);fm();await d(1000);ok=await waitVolante()}return ok}
