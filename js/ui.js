...
async function waitVolante(){for(var w=0;w<40;w++){var el=document.getElementById("n01")||document.querySelector("a[id^=n]")||document.querySelector(".dezena")||document.querySelector("[ng-click*=dezena]");if(el&&el.offsetParent!==null){console.log("[B2B] Volante encontrado!");return!0}fm();await d(1000);console.log("[B2B] Aguardando volante... ("+w+"/40)")}return!1}
async function navLoteria(){var hash=window.location.hash||"";if(hash.indexOf(LOTTERY_URL)>=0){console.log("[B2B] Ja na pagina correta");var ok=await waitVolante();if(ok)return!0}
console.log("[B2B] Metodo 1: Angular $location...");try{var inj=angular.element(document.body).injector();if(inj){var loc=inj.get("$location");var scope=angular.element(document.body).scope();if(loc&&scope){scope.$apply(function(){loc.path("/"+LOTTERY_URL)});console.log("[B2B] Angular navegou para /"+LOTTERY_URL);await d(4000);fm();await d(1000);var ok=await waitVolante();if(ok)return!0}}}catch(e){console.log("[B2B] Angular falhou: "+e.message)}
console.log("[B2B] Metodo 2: Clicar link do menu...");var links=document.querySelectorAll("a");for(var i=0;i<links.length;i++){var href=(links[i].getAttribute("href")||"").toLowerCase();var txt=links[i].textContent.trim().toLowerCase();if(href.indexOf(LOTTERY_URL)>=0||href.indexOf("#/"+LOTTERY_URL)>=0){console.log("[B2B] Clicando: "+txt+" ("+href+")");links[i].click();await d(4000);fm();await d(1000);var ok=await waitVolante();if(ok)return!0}}
console.log("[B2B] Metodo 3: location.href...");window.location.href="https://www.loteriasonline.caixa.gov.br/silce-web/#/"+LOTTERY_URL;await d(5000);fm();await d(1000);return await waitVolante()}
